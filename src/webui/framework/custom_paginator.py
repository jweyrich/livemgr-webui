...
class CustomPaginator(Paginator):
	def __init__(self, request, queryset, per_page=25, page_number=None):
		...
		if page_number == None:
			page_number = self._get_page_from_request(request)
	...
	def _paginate(self, page_number):
		...
		self._limit_page_range(page_number, 5)
		return result
	def _get_page_from_request(self, request):
		try:
			page = int(request.GET.get('page', '1'))
		except ValueError:
			page = 1
		return page
	def _limit_page_range(self, page_number, range_gap=5):
